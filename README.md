...
| 合盖游戏位：手机屏幕 ↔ 最高控制件 | **+2.5mm** 净空 |
| 合盖手机模式：deck ↔ 手机 | **+8.8mm** |
| 整机厚度 | 合盖游戏 **34.1mm**；手机模式 **42.4mm**；**拔掉底座当日常壳 22.5mm** |

**日常模式**：拧下铰链的 M4 螺杆 → 取下带按键的底座 → 剩下盖框 + 托盘 + 手机 = **22.5mm 厚**的保护壳，屏幕朝上正常用。这是这个结构天然的"两用"方式，不需要额外零件（建议用 M4 手拧螺钉或插销，方便随时拆装）。
...
演示页是**零依赖**的（自己写的 WebGL 渲染器，不用 CDN）：拖滑块改 θ/φ、一键三形态、播放翻转动画、隐藏部件看内部、**日常模式一键看**（拔掉底座的样子）、换屏幕内容（可玩的游戏 / 主屏幕 / 你自己的图 / 灭屏）、CRT 扫描线滤镜。
...
| # | 玩家 | 分数 | 提交 |
|---|---|---|---|
| 🥇 | [@Li-Mingshuang](https://github.com/Li-Mingshuang/flipdeck/issues/1) | **8880** | 2026-09-13 |

## 🎮 打砖块（`break`）

_还没有成绩 —— 来占个第一？_

_最后更新：2026-09-13 18:29（UTC）_
<!-- LEADERBOARD:END -->
...
- **合盖 34–42mm 厚**：这是"合盖还要能当手机用"的物理代价；**日常用就拧下螺杆取走底座（22.5mm）**，想要更薄只能改架构（控制模块做成磁吸可拆）
