...
1. **托盘轴放在托盘板的中面上** → 翻转 180° 后托盘仍占同一层空间，只有手机从盖内侧换到盖外侧。否则要么盖变厚，要么合盖时手机撞底座。
   托盘还带一圈 **7.4mm 围边**（比手机屏幕高 0.7mm）：φ=0 时朝内、φ=180 时朝外，所以**两个方向手机都是"嵌"在托盘里的**，不会出现"一整块贴在框背面的板砖"。围边在 ±X 两端各开一个手指缺口方便取手机；围边留在托盘 160×74.5 轮廓内，翻转扫掠半径 37.85mm（横梁在 39mm，仍留 1.15mm）。
...
> 在演示页里玩完小游戏（**跑酷 / 俄罗斯方块 / 打砖块 / 贪吃蛇 / Flappy**），点「提交成绩」会开一个预填好的 GitHub Issue；Actions 自动汇总到这里，也会画在演示页的手机屏幕上（屏幕下拉框选「🏆 排行榜（实时）」）。同一玩家每款游戏只记最好成绩。
...
_最后更新：2026-09-26 08:37（UTC）_
<!-- LEADERBOARD:END -->

## 模块化：换个底座就变形态

**铰链接口就是模块标准**（铰链销孔、凸耳位置、贴合面高度 19mm、磁铁位、密封槽都固定不变）。拔掉 M4 销 → 取下底座 → 换上另一个底座 → 插回销，10 秒完成；盖子、托盘、手机全都不用动。

| 底座 | 内容 | 用途 |
|---|---|---|
| `deck`（手柄底座） | 2 摇杆 + 11 按键 + 打印开关架 | 玩游戏 |
| `deck_keyboard`（键盘底座） | 一个**买来的迷你蓝牙键盘模块**（约 150×60×7.5mm，自带电池/蓝牙/充电口）的舱位 | **vibecoding**：手机在上盖当屏幕、键盘在下座 = 迷你笔记本 |
| 不装底座（日常） | 只剩盖框 + 托盘 + 手机 | 当普通保护壳（22.5mm 厚） |

键盘底座设计要点：舱体 152.4×62.4，模块放进去后**键面比边缘低 0.5mm**；前缘 0.8mm 挡边防倒置掉出；左侧壁开 14×6 口给模块插线充电；底部掏空省料。不需要触摸板（手机屏幕本身就是触摸屏），也不需要自研电路（键盘模块自带 BLE）—— **只需要打印一个底座**。

演示页里点「底座：手柄 / 键盘」可以直接切换看两种形态。
