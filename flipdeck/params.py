...
# ----------------------------------------------------------------------------- 模块化：键盘底座
# 接口标准（任何底座变体都必须满足，否则合不上盖）：
#   * 铰链轴线固定在 (HINGE_Y, HINGE_Z)，底座凸耳 |x| ∈ HINGE_X_DECK
#   * 贴合面（与盖内表面接触的四周边缘）必须正好在 z = DECK_H
#   * 磁铁位 / 密封条槽 / 铰链腹板位置一致
KB_MODULE = (150.0, 60.0, 7.5)   # 迷你蓝牙键盘模块：长 × 宽 × 厚（自带电池与蓝牙，买现成）
KB_FIT = 1.2                     # 舱体单边间隙
KB_TOP_GAP = 0.5                 # 模块顶面比贴合面低多少（防误触）
KB_LIP = 0.8                     # 前缘挡边（防止倒过来掉出）
KB_CHARGE_CUT = (14.0, 6.0)      # 侧壁充电口（给模块插线用）
